...
def acceptance_probability(position, new_position, potential, k, theta, chi):
    x = position.reshape((-1,1))
    y = new_position.reshape((-1,1))
    print(x.shape)
    print(np.cos(k*theta))
    print(potential(x))
...
def final_state(initial_distribution, step_size, max_iterations = 10000,
    ...
    C_inv = np.diag([1/2] + list(np.ones(dimension-1)))
    theta = np.arccos(np.eye(dimension) - (step_size)**2/2 * C_inv)
    chi = (np.eye(dimension) - ((step_size**2)/4)*C_inv)*(C_inv)
...
potential = lambda x : 0.5 * x.T @ C_inv @ x

print(final_state(multivariate_normal(cov=np.eye(5), allow_singular = True), 
                  0.01, max_iterations = 10, flow_time=1.0, 
                  grad_potential = grad_potential, potential = potential))
