...
def test_nuts6():
    ...
    print(f'Shape of samples is {samples.shape}')
...
for i in range(SAMPLE_NUMBER):
    ...
    samples, lnprob, epsilon = nuts6(f, M, Madapt, theta0, delta)
    trajectories.append(samples)
    print(f'Functions called: {c.c}')

np.save(f'NUTS_dim={D}_samples={SAMPLE_NUMBER}_M={M}_delta={delta}_Madapt={Madapt}', trajectories)
samples = samples[1::10, :]
print('Percentiles')
print (np.percentile(samples, [16, 50, 84], axis=0))
print('Mean')
