# -*- coding: utf-8 -*-
"""
Created on Tue Aug 17 12:36:49 2021
...
def final_state(initial_distribution, step_size, max_iterations = 10000,
    ...
    Metrolpolis Hastings method with ULA chain update
    
    X -> X + root(2h)Z - h*grad_potential(X)
    
    density = C * np.exp(-potential(x))
    ...
    for i in range(max_iterations):
        ...
        grad_potential_old = grad_potential(x)
        
        y = x + root_two_h * np.random.randn(dim) - h * grad_potential_old
        
        grad_potential_new = grad_potential(y)
        
        accept_prob = min(1, np.exp(-potential(y) - np.dot(x-y+h*grad_potential_new, x-y+h*grad_potential_new)/(4*h) 
                                    + potential(x) + np.dot(y-x+h*grad_potential_old, y-x+h*grad_potential_old)/(4*h)))
...
#    return (1/upper_hessian_bound) * min(1/dimension, 1/np.sqrt(dimension * condition_number))
#     return (1/upper_hessian_bound) * 1/dimension
    return (1/upper_hessian_bound) * 1/np.sqrt(dimension * condition_number)
#    return np.power(dimension, -0.25)
...
for i in range(noOfSampledPoints):
    final_state = MALA(gradf,f,h,multivariate_normal(cov= 1/L * np.eye(2)) ,maxIterations = maxIterations)
...
fig.suptitle('MALA-Comparison of histograms along slices')

axes[0].set_title('X axis')
axes[0].hist(xs,bins=100,range=(-5,5),density=True,alpha=0.6,label='MALA')
axes[0].hist(empirical_xs,bins=100,range=(-5,5),density=True,alpha=0.6,label='target')
axes[0].legend()

axes[1].set_title('Y axis')
axes[1].hist(ys,bins=100,range=(-5,5),density=True,alpha=0.6,label='MALA')
axes[1].hist(empirical_ys,bins=100,range=(-5,5),density=True,alpha=0.6,label='target')
axes[1].legend()


plt.savefig("MALA-Comparison of histograms along slices")
plt.show()
...
# Plot a 3D surface
ax.set_title("MALA 2D Histogram")
ax.plot_surface(Xbins, Ybins, counts,label='MALA')
...
ax.legend([fake2Dline_ula,fake2Dline_target], ['MALA','Target'], numpoints = 1)

plt.savefig('MALA 2D Histogram')
plt.show()

'''
#%%
def states(gradf,f,h,initial_distribution ,maxIterations = 10000):
    x = initial_distribution.rvs()
    root_two_h = np.sqrt(2*h)
    dim = x.shape[0]
    ...
    for i in range(maxIterations):
        ...
        y = x + root_two_h * np.random.randn(dim) - h * gradf_old
        
        gradf_new = gradf(y)
        
        accept_prob = min(1, np.exp(-f(y) - np.dot(x-y+h*gradf_new, x-y+h*gradf_new)/(4*h) + f(x) + np.dot(y-x+h*gradf_old, y-x+h*gradf_old)/(4*h)))
...
def three_fourth_quantile_mixing(h,initial_distribution,
    ...
    for i in range(max_iterations):
        ...
        accept_prob = min(1, np.exp(-potential(y) 
                                    - np.dot(x-y+h*grad_potential_new, x-y+h*grad_potential_new)/(4*h) 
                                    + potential(x) 
                                    + np.dot(y-x+h*grad_potential_old, y-x+h*grad_potential_old)/(4*h)))
...
for samples in range(5):
    states = np.array(MALA_states(gradf,f,h,multivariate_normal(cov= 1/L * np.eye(2)) ,maxIterations = maxIterations))
...
#plt.xscale('log')
axes[0].plot(range(maxIterations),mean_tv_errors,label = f'MALA h = {np.round(h,5)}')
...
#plt.xscale('log')
axes[1].plot(np.linspace(0,T_max,num=maxIterations),mean_tv_errors,label = f'MALA h = {np.round(h,5)}')
...
plt.plot(range(maxIterations),mean_tv_errors,label = f'MALA h = {h}')
...
plt.savefig("MALA_TVError_Iteration")
plt.show()
#%%
T_max = 100
...
plt.plot(np.linspace(0,T_max,num=maxIterations),mean_tv_errors,label = f'MALA h = {h}')
...
plt.legend()
plt.title("Discrete TV error Vs time")
plt.savefig("MALA_TVError_time")
plt.show()

#%%
import bisect

def MALAMixing(gradf,f,h,initial_distribution,
    ...
    for i in range(maxIterations):
        ...
        accept_prob = min(1, np.exp(-f(y) - np.dot(x-y+h*gradf_new, x-y+h*gradf_new)/(4*h) + 
                                    f(x) + np.dot(y-x+h*gradf_old, y-x+h*gradf_old)/(4*h)))
...
plt.savefig('tmix vs dim loglog - MALA')  
plt.show()
plt.plot(dimensions,mixing_times,'og')  
plt.savefig('tmix vs dim - MALA')  
plt.show()
#%%
m,b = np.polyfit(dimensions,mixing_times,1)
plt.title('MALA tmix 0.75 quantile - tmix vs dim')
plt.plot(dimensions,m*dimensions + b)
plt.plot(dimensions,mixing_times,'og')
plt.xlabel('Dimension')
plt.ylabel('Tmix')
plt.savefig('MALA tmix vs dim - linear')
plt.show()
m
...
slope, intercept, r_value, p_value, std_err = linregress(np.log(np.arange(5,16)), np.log(mixing_times))
...
plt.savefig('tmix  vs delta, d = 15, MALA')
plt.show()
#%%
...
plt.savefig('tmix vs delta loglog d=15 MALA')
plt.plot(newX, myExpFunc(newX, *popt), 'b')
#%%
popt
#%%
...
for samples in range(50):
    states = np.array(MALA_states(gradf,f,h,multivariate_normal(cov= 1/L * np.eye(2)) ,maxIterations))
...
plt.yscale('log')
plt.plot(range(maxIterations+1),mean_l1_errors,label = f'MALA h = {np.round(h,5)}')
...
plt.legend()
plt.title('MALA - Mean l1 error- Logistic regression')
plt.ylabel('l1 Error')
plt.xlabel('Iteration')
plt.savefig('MALA-Mean l1 error, no pre conditioning')
'''
